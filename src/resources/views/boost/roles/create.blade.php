<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <h1 class="h1 ms-3">Criar Função</h1>
    </x-slot>
    ...
    <form action="{{ route('boost.roles.store') }}" id="role-form" method="POST">
        @csrf
        ...
        <div class="form-group">
            ...
            @foreach ($permissions as $permission)
                <x-switch :active="false" name="permissions[{{ $permission->id }}]" value="{{ $permission->id }}"
                    id="{{ $permission->id }}">
                    {{ $permission->name }}
                </x-switch>
            @endforeach
