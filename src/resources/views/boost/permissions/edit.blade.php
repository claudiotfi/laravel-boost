<x-app-layout>
    ...
    <x-slot name="tools">
        <x-btn-close route="boost.permissions.index"></x-btn-close>
        <x-btn-save></x-btn-save>
    </x-slot>

    <form action="{{ route('boost.permissions.update', $permission->id) }}" id="permission-form" method="POST">
        @csrf
        @method('PUT')
        ...
        <button type="submit" id="permission-form-submit" class="hidden"></button>
    </form>
